...
> **دليل التحقق:** `42/42` اختبار ناجح (`tests/`) — يشمل اختبار E2E حقيقي: صورة عربية → OCR → تحليل → PDF → تشفير → تحقق، واختبارَي حصانة ضد انهيار الخطوط على بيئات النشر.
...
│  └─ ui.py                # طبقة الواجهة المشتركة: CSS مخصص + شارة نطاق الخطر (بلا أي منطق خلفي)
...
│  ├─ fonts/               # NotoNaskhArabic-Regular.ttf — خط عربي مضمّن (ضمان توليد PDF بلا خطوط نظام)
...
- `pytest tests/` → **42 passed** (0 فشل) — آخر تشغيل كامل نظيف.
- **SRE 2026-09-22 — إصلاح انهيار S5 على بيئات بلا خطوط نظام:** `FileNotFoundError` من `_resolve_font()` على حاوية Streamlit Cloud أُعيد إنتاجه محلياً بمحاكاة قائمة خطوط فارغة. السبب الجذري: اعتماد `FONT_CANDIDATES` على مسارات نظام + وجود **تعريفين مكررين** للدالة (الثاني يلغي الأول الذي كان يحتوي حل التحميل التلقائي الهش) + غياب أي خط مضمّن. الإصلاح المجهري: تحزيم `data/fonts/NotoNaskhArabic-Regular.ttf` (Noto Naskh Arabic، OFL، 247KB) وإضافته آخر مرشح في `FONT_CANDIDATES` + حذف التعريف الميت (شمل إزالة استيرادي urllib/tempfile) + اختبارا حصانة في `test_report.py` يحاكيان بيئة بلا خطوط نظام. **42/42 خضراء**.
- **UI/UX 2026-09-22 — واجهة احترافية (جراحة بصرية بلا لمس منطق خلفي):** `core/ui.py` جديد يركّز CSS الموحّد (بطاقات Metrics مدوّرة، تنبيهات بحدود ملونة، أزرار/حقول/أكورديون/شريط تقدم، حاوية أوسع) + بطاقة مقدمة `.tf-intro` + شارة نطاق خطر ديناميكية `tf-risk` فوق مؤشرات S3. بقيت جميع المفاتيح و`ingest_form` وعناوين الأقسام كما هي — **42/42 خضراء** وإقلاع خادم حي `ok`.
- **UI/UX 2026-09-22 — إصلاح الألوان عالية التباين والقراءة (High Contrast & Readability):** أُعيدت لوحة `core/ui.py` لتشتق من **متغيرات سمة Streamlit الأساسية** (`--text-color`/`--background-color`/`--secondary-background-color`/`--primary-color`) فتتبع الوضعين Light/Dark تلقائياً — نصوص `tf-ink` بعكس الخلفية، خلفيات البطاقات من `secondary-background-color`، حدود شفافة من النص. عناوين الأقسام بخط عريض + شريط تمييز `tf-brand` (Dark Navy فاتح / أزرق فاتح داكن). ألوان شارات الخطر إلى درجات 700 (`#15803d`/`#a16207`/`#c2410c`/`#b91c1c`) — تباين أبيض ≥ 4.2:1 (WCAG AA) مع إطار تمييزي. لم يُلمس سوى `core/ui.py` — **42/42 خضراء**.
